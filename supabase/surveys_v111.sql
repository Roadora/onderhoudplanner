...
create table if not exists public.surveys (
  ...
  constraint surveys_appointment_fk foreign key (organization_id,appointment_id)
...
create table if not exists public.survey_photos (
  ...
  created_at timestamptz not null default now(),
  constraint survey_photos_appointment_fk foreign key (organization_id,appointment_id)
    references public.appointments(organization_id,id) on delete cascade
);
...
using(bucket_id='opname-fotos' and public.can_access_survey_v110((storage.foldername(name))[1]::uuid,(storage.foldername(name))[2]));
create policy "opname_fotos_insert" on storage.objects for insert to authenticated
with check(bucket_id='opname-fotos' and public.can_access_survey_v110((storage.foldername(name))[1]::uuid,(storage.foldername(name))[2]));
...
returns table(appointment_id text,purpose text,scope text,findings text,technical_notes text,status text,updated_at timestamptz)
...
create or replace function public.get_survey_v110(p_organization_id uuid,p_appointment_id text)
returns table(appointment_id text,purpose text,scope text,findings text,technical_notes text,status text,updated_at timestamptz)
language sql stable security definer set search_path=public,pg_temp as $$
 select s.appointment_id,s.purpose,s.scope,s.findings,s.technical_notes,s.status,s.updated_at
...
create or replace function public.upsert_survey_v110(p_organization_id uuid,p_appointment_id text,p_purpose text,p_scope text,p_findings text,p_technical_notes text,p_status text)
...
create or replace function public.list_survey_photos_v110(p_organization_id uuid,p_appointment_id text)
...
create or replace function public.register_survey_photo_v110(p_organization_id uuid,p_appointment_id text,p_storage_path text,p_caption text default '')
...
returns text language plpgsql security definer set search_path=public,pg_temp as $$
declare v_path text; v_appointment text;
begin
...
revoke all on function public.can_access_survey_v110(uuid,text),public.list_surveys_v110(uuid),public.get_survey_v110(uuid,text),public.upsert_survey_v110(uuid,text,text,text,text,text,text),public.list_survey_photos_v110(uuid,text),public.register_survey_photo_v110(uuid,text,text,text),public.delete_survey_photo_v110(uuid,uuid) from public;
grant execute on function public.can_access_survey_v110(uuid,text),public.list_surveys_v110(uuid),public.get_survey_v110(uuid,text),public.upsert_survey_v110(uuid,text,text,text,text,text,text),public.list_survey_photos_v110(uuid,text),public.register_survey_photo_v110(uuid,text,text,text),public.delete_survey_photo_v110(uuid,uuid) to authenticated;
