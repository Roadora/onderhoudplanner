...
render();
function agendaTimelineCard(a){
  const s=a.systemId ? systemById(a.systemId) : null;
  const c=(a.customerId ? customer(a.customerId) : null) || (s ? customer(s.customerId) : {}) || {};
  const type=a.type || 'onderhoud';
  const title=appointmentTitle(type);
  const start=a.time || 'Tijd';
  const subtitle=s ? `${s.brand} ${s.model}` : (a.note || 'Afspraak');
  const address=c.address || '';
  return `<article class="agenda-time-card ${type}" onclick="nav('newAppointment',{appointmentId:'${a.id}',back:'agenda'})">
    <div class="time-col">
      <strong>${start}</strong>
    </div>
    <div class="time-content">
      <div class="row between">
        <div>
          <p class="title">${appointmentIcon(type)} ${title}</p>
